...
with open('input/lista_erros_item_0.json', 'r', encoding='utf-8-sig') as openfile:
...
async def main():
    ...
    conc_req = 5
    now = time.time()
    await gather_with_concurrency(conc_req, *[get_async(i, session, results) for i in prepositions])
    time_taken = time.time() - now

    print(time_taken)
    await session.close()

    with open("output/lista_preposicoes_3.json", "w", encoding='utf8') as outfile:
